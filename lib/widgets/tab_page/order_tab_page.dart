...
class _OrderTabPageState extends State<OrderTabPage> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.status == 'Tất cả') {
      ...
      orderStream = _ordersController.getOrdersByStatusStream(userId, widget.status);
    }    
                ...
                onReviewPressed: () {
                  ...
                  if (order.status == OrdersController.STATUS_COMPLETED) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Tính năng đánh giá đang được phát triển'))
                    );
                  }
                },
                onDetailsPressed: () {
